...
This document presents the prepare and process stages of the data analysis process,for spectroscopic data from the project **The structure and interaction of silica in plant cell walls** as well as exploratory data analysis results, as well as modelling using genetic feature selection algorithms and multiple ordinary least squares regression (MOLS).  


# Prepare 
## Loading data into `R`

An object called `names` is created, which is a `list`, in which the names of the files in the current directory which have the `.CSV` extension will be saved:
...
A function useful to import data from `.CSV` files is the function `read.csv`. It takes the data inside the original files and stores it in a `data.frame` object inside `R`.

Since in this case, each file represents one measurement, reading and storing 318 files, one at a time is time consuming.

To solve this, the function `lapply` can be used, which applies the same function to each item of a `list`. The `data.frame` that comes from each file, will be saved in each individual position of the new list: `spectra.list`.
...
## Creating a table with all samples information
...
The following code: `spectra.list[[1]][1]` selects the first element of the list `spectra.list` using the index operator for lists `[[1]]`. Inside this first position of the list there is a `data.frame`. the first column of that `data.frame` is then selected using the index operator `[1]`:
...
The following code selects only the second column of each `data.frame` and saves it inside each slot of the list `spectra.list2`
...
```
Finally, we assign the cleaned names to our data frame, `spectra.df`
...
Since each sample was measured three times, The mean spectra for the three measurements of each sample can be calculated as follows:

  A vector is created, with the individual ID for each sample. The function `unique()` is used to store all ID without repetitions.
...
Once the ID for each sample is identified and stored inside the `search.vector` it is possible to search for $\dfrac{318}{3}=106$ triads of measurements. Once their position is know, the mean spectra for each sample can be calculated.
...
Then we can loop through columns indexing each wave number, one after another, in each iteration of the loop, or each time `j` changes its value. Possible values for `j` are defined at `in 1:length(ncol(range1))`. where `ncol` gives us the number of columns of `range1` or the number of wave numbers present along the ROI.

At the same time, `i` takes values at each one of the numbers present between `1` and `nrow(mean)`, which is the number of samples, or 106.
...
it is useful to save both, the corrected and mean spectra in .csv files for further analysis:

```{r}
write.csv(corrected, file = 'corrected.csv')
...
# the same size as metadata$sample
...
```{r matching for metadata$sample and rownames$corrected selected by  positions[[i]], message=F}
...
```{r searching the index number for metadata samples in corrected, message = F}
...
a copy of this table can be saved for further analyisis:

```{r saving metadata table as a file}
...
```{r tidying up the data frame for hclust}
...
```{r hkmeans calculation, message = F}
library(factoextra)
...
```{r dendrogram visualization ggplot2, message =F, warning=FALSE}
fviz_dend(res.hk,
          cex = 0.5,
          palette = "jco", 
          rect = TRUE, 
...
```{r checking for Si NA in all samples}
...
Once we have selected the samples that have both spectra and silicon content available, this data set is ready to perform modelling. Also we can show that every sample has classification information as follows:

```{r checking for class NA in all samples}
which(is.na(metadata.si$sample))
```
`integer(0)` means that there are not missing values for the classification column, `metadata.si$sample`.
...
```{r creating a vector with silicon content for all samples}
SiTable <- data.frame(sample = metadata.si$sample,
                      Si = metadata.si$Si)

```

then, we can extract the corresponding spectra for each sample:

```{r extracting spectra for silicon selected samples from all samples}
...
```{r selection of variables for all samples, cache = T}
...
```{r selected variables plot for all samples}
# par(mfrow=c(4,3))
...
```{r calculating PLS with mdatools}
library(mdatools)
...
```{r leaves mean and corrected spectra plot,  fig.cap="Mean and corrected spectra for leaves samples"}
par(mfrow = c(1,2))
