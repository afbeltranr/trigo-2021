...
```{r printing the column names of metadata matrix}
colnames(metadata)
```
where:
...
Since nowwhe know where in the spectral matrix is each sample of the `metadata` data set, we can add a new column that relates where is each sample of `metadata` in the `corrected` matrix:
...
```{radding the index number for metadata samples in corrected}
for(i in 1:length(metadata$sample)) {
