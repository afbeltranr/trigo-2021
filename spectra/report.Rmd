...
*Then, we asign to this empty matrix, `mean`, its corresponding column and row names:
```{r}
...
* the multiplication by $0.1$ is the normalization for this spectra.
Now we can use the `rubberband` method, and setting the noise to $1\cdot10^{-4}$ allows us to take advantage of the low noise presented for this spectra. (a noise of `noise=4` sets an interval of 2 times standard deviations).
